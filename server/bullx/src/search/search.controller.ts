...
export class SearchController {
    ...
    constructor(private readonly searchService: SearchService, private readonly configService: ConfigService) {
        ...
        this.marketStackApiKey = this.configService.get<string>('MarketStack_API_KEY');
        this.marketStackEpURL = this.configService.get<string>('MarketStack_API_URL');
    ...
    async searchTicker(@Query('ticker') ticker: string) {
        ...
        console.log('search ticker:', ticker);
        try {
            return this.searchService.searchTicker(ticker, this.FMPApiKey, this.FMP_API_URL_BASE);
        } catch (error) {
            throw new InternalServerErrorException(`Failed to fetch ticker search results: ${error.message}`);
        }
    }
}