...
export class ScannerService implements ScannerServiceModel {
  ...
  getMostActive(
    ...
    return this.httpService
      ...
      .pipe(
        ...
        map((response: AxiosResponse<mostActiveStock>) => {
          this.logger.log(response.data);
          return this.scannerMapService.mapScannerInfo(
            response.data,
            'mostActiveStock' as 'mostActiveStock',
          );
        }),
      );
  }
  ...
  getMostGainers(
    apiKey: string | undefined,
    epURL: string | undefined,
  ): Observable<mostGainersStock> {
    if (!apiKey || !epURL) {
      throw new Error('Missing FMP_API_KEY or FMP_API_URL in config.');
    }
    this.logger.log(`${epURL}/stock/gainers?${apiKey}`);
    return this.httpService
      .get(`${epURL}/stock/gainers`, {
        params: { apikey: apiKey },
        responseType: 'json',
      })
      .pipe(
        ...
        map((response: AxiosResponse<mostGainersStock>) => {
          ...
          return this.scannerMapService.mapScannerInfo(
            response.data,
            'mostGainerStock' as 'mostGainerStock',
          );
        }),
      );
  }
  ...
  getMostLosers(
    apiKey: string | undefined,
    epURL: string | undefined,
  ): Observable<mostLosersStock> {
    if (!apiKey || !epURL) {
      throw new Error('Missing FMP_API_KEY or FMP_API_URL in config.');
    }
    this.logger.log(`${epURL}/stock/losers?${apiKey}`);
    return this.httpService
      .get(`${epURL}/stock/losers`, {
        params: { apikey: apiKey },
        responseType: 'json',
      })
      .pipe(
        ...
        map((response: AxiosResponse<mostLosersStock>) => {
          ...
          return this.scannerMapService.mapScannerInfo(
            response.data,
            'mostLoserStock' as 'mostLoserStock',
          );
        }),
      );
  }

  getTopGainersByPercent(
    apiKey: string | undefined,
    epURL: string | undefined,
  ): Observable<IStockPercent[]> {
    if (!apiKey || !epURL) {
      throw new Error('Missing FMP_API_KEY or FMP_API_URL in config.');
    }
    this.logger.log(`${epURL}/stock_market/gainers?${apiKey}`);
    return this.httpService
      .get(`${epURL}/stock_market/gainers`, {
        params: { apikey: apiKey },
        responseType: 'json',
      })
      .pipe(
